...
#We also grab some kinetic energies associated with the mean (m=0) motions
#  mke  = lut[405]
...
#We also grab magnetic energy
  me  = lut[1101]
  rme = lut[1102]  # ME associated with mean radial motion
  tme = lut[1103]  # ME associated with mean theta motion
  pme = lut[1104]  # ME associated with mean azimuthal motion
  
#We also grab some kinetic energies associated with the mean (m=0) motions
  mrme = lut[1106]  # KE associated with mean radial motion
  mtme = lut[1107]  # KE associated with mean theta motion
  mpme = lut[1108]  # KE associated with mean azimuthal motion
...
# To begin with, let's plot the total, mean, and fluctuating kinetic energy density during the initial transient phase, and then during the equilibrated phase.
  ...
  ax.plot(time, gavgs[:,ke], label='Kinetic')
  ax.plot(time, gavgs[:,me], label='Magnetic')
...
#  ax.set_xlim([0,0.2])
  ...
  ke_m0 = gavgs[:,mrke] + gavgs[:,mtke] + gavgs[:,mpke]
  ke_mn = gavgs[:,ke] - ke_m0
  fig, ax = plt.subplots(ncols=1, figsize=sizetuple)
  ax.plot(time, gavgs[:,ke], label='Total')
  ax.plot(time, ke_m0, label='Axisymmetric')
  ax.plot(time, ke_mn, label='Non-axisymmetric')
...
#  ax.set_xlim([0,0.2])
  ...
  me_m0 = gavgs[:,mrme] + gavgs[:,mtme] + gavgs[:,mpme]
  me_mn = gavgs[:,me] - me_m0
  fig, ax = plt.subplots(ncols=1, figsize=sizetuple)
  ax.plot(time, gavgs[:,me], label='Total')
  ax.plot(time, me_m0, label='Axisymmetric')
  ax.plot(time, me_mn, label='Non-axisymmetric')
  ax.legend(loc='upper left', shadow=True)
#  ax.set_xlim([0,0.2])
  ax.set_yscale('log')
  ax.set_title('Entire Time-Trace')
  ...
  mnratio = me_mn / ke_mn
  fig, ax = plt.subplots(ncols=1, figsize=sizetuple)
  ax.plot(time, ratio, label='Total')
#  ax.plot(time, m0ratio, label='Axisymmetric')
#  ax.plot(time, mnratio, label='Non-axisymmetric')
  ax.legend(loc='upper left', shadow=True)
#  ax.set_xlim([0,0.2])
#  ax.set_yscale('log')
  ax.set_title('Entire Time-Trace')
