...
def plot_each_Rakesh_examples(d):
...
#  Convert equitorial section
  org_Equatorial_Slices = source_dir + dir_Equatorial_Slices
  dest_Equatorial_Slices = dest_dir + dir_Equatorial_Slices
  filelist = []
  filelist = os.listdir(org_Equatorial_Slices)
  nfile=len(filelist)
  max_step = int(filelist[0])
  lastfile = filelist[0]
  for fname in filelist:
    ...
    if(istep > max_step):
      max_step = istep
      lastfile = fname
  Equatorial_Slices_lastfile = lastfile
  print('Last file in ', org_Equatorial_Slices, ':   ', Equatorial_Slices_lastfile)
  if(os.path.isdir(dest_Equatorial_Slices) == False):
    os.makedirs(dest_Equatorial_Slices)
  
  org_filepath = org_Equatorial_Slices + Equatorial_Slices_lastfile
  dest_filepath = dest_Equatorial_Slices + Equatorial_Slices_lastfile
  shutil.copy(org_filepath, dest_filepath)
  each_convert(dest_filepath)
...
#  make image directory
  ...
  init_time = s_plot_G_Avgs_rakesh(dest_G_Avgs)
  ...
  s_plot_Shell_Slices_rakesh(dest_Shell_Slices, file_name, init_time)
  
  file_name = dest_dir + Shell_Spectra_caption_prefix
  s_plot_Shell_Spectra_rakesh(dest_Shell_Spectra, file_name, init_time)
...
if __name__ == '__main__':
  ...
  if 2 <= len(args):
    for d in args[1:]:
      print(d)
      plot_each_Rakesh_examples(d)
  else:
    print('set directory names below ryadav')
